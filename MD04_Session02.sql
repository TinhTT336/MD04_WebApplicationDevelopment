...
SELECT * FROM session02.category where status=1;
SELECT * FROM session02.product where sale_price>0;
SELECT id, product_name, price, sale_price, ((1-sale_price/price)*100)as percent_price FROM session02.product where sale_price>0;
-- ALTER TABLE product MODIFY percent_price float(10,2);
UPDATE category SET status=0 where category_id=1;
