...
select DMKhoa.TenKhoa, max(DMSV.HocBong) as HocBongMax from DMSV JOIN DMKhoa ON DMSV.MaKhoa=DMKhoa.MaKhoa GROUP BY DMSV.MaKhoa;
...
trong lần thi 1.*/
select DMMH.MaMH,DMMH.TenMH, COUNT(CASE WHEN KetQua.Diem>=5 THEN 1 END) SoSVDau,COUNT(CASE WHEN KetQua.Diem<5 THEN 1 END) SoSVRot 
from KetQua JOIN DMMH ON DMMH.MaMH=KetQua.MaMH WHERE KetQua.LanThi=1 GROUP BY DMMH.MaMH,DMMH.TenMH;

-- 39.Cho biết sinh viên nào có học bổng cao nhất.
select DMSV.MaSV,DMSV.HoSV,DMSV.TenSV,MAX(HocBong) as HocBongMax from DMSV GROUP BY MaSV HAVING HocBongMax=(select MAX(HocBong) from DMSV);

-- 40.Cho biết sinh viên nào có điểm thi lần 1 môn cơ sở dữ liệu cao nhất.
select DMSV.MaSV,DMSV.HoSV,DMSV.TenSV,KetQua.diem from DMSV
JOIN KetQua ON DMSV.MaSV=KetQua.MaSV WHERE KetQua.LanThi=1 and diem=(select MAX(diem) from KetQua);

-- 41.Cho biết sinh viên khoa anh văn có tuổi lớn nhất.
select DMSV.MaSV,DMSV.HoSV,DMSV.TenSV,DMKhoa.TenKhoa, Year(current_date())- Year(DMSV.NgaySinh) as Tuoi from DMSV 
JOIN DMKhoa ON DMKhoa.MaKhoa=DMSV.MaKhoa  
HAVING Tuoi=(select MAX(YEAR(CURRENT_DATE()) - YEAR(DMSV.NgaySinh)) from DMSV WHERE TenKhoa='anh văn');

-- 42.Cho biết khoa nào có đông sinh viên nhất.
select DMKhoa.MaKhoa,DMKhoa.TenKhoa, COUNT(*) as TongSV from DMSV JOIN DMKhoa ON DMKhoa.MaKhoa=DMSV.MaKhoa 
GROUP BY MaKhoa HAVING COUNT(*) =(SELECT MAX(TongSV) FROM (SELECT COUNT(*) AS TongSV FROM DMSV GROUP BY MaKhoa) AS SubQuery);

-- 43.Cho biết khoa nào có đông nữ nhất.
select DMKhoa.MaKhoa,DMKhoa.TenKhoa, COUNT(*) as TongSV from DMSV  JOIN DMKhoa ON DMKhoa.MaKhoa=DMSV.MaKhoa WHERE DMSV.Phai='nữ'
GROUP BY MaKhoa HAVING COUNT(*) =(SELECT MAX(TongSV) FROM (SELECT COUNT(*) AS TongSV FROM DMSV GROUP BY MaKhoa) AS SubQuery);

-- 44.Cho biết môn nào có nhiều sinh viên rớt lần 1 nhiều nhất.
select DMMH.MaMH,DMMH.TenMH, COUNT(*) as TongSVRot from KetQua JOIN DMMH ON DMMH.MaMH=KetQua.MaMH
WHERE KetQua.LanThi=1 and KetQua.diem<5 GROUP BY MaMH  ORDER BY COUNT(*) DESC LIMIT 1;

-- 45.Cho biết sinh viên không học khoa tin hoc có điểm thi môn van pham lớn hơn điểm thi van pham của sinh viên học khoa Tin Học.
select DMSV.MaSV,DMSV.HoSV,DMSV.TenSV,DMKhoa.TenKhoa,KetQua.MaMH,KetQua.diem from DMSV 
JOIN DMKhoa ON DMKhoa.MaKhoa=DMSV.MaKhoa
JOIN KetQua ON DMSV.MaSV=KetQua.MaSV WHERE TenKhoa NOT IN ('Tin Học') and KetQua.MaMH=03;



-- 46.Cho biết sinh viên có nơi sinh cùng với Hải.
select  DMSV.MaSV,DMSV.HoSV,DMSV.TenSV, NoiSinh from DMSV WHERE NoiSinh=(select NoiSinh from DMSV WHERE TenSV='Hải') and TenSV<>'Hải';

-- 47.Cho biết những sinh viên nào có học bổng lớn hơn tất cả học bổng của sinh viên thuộc khoa anh văn
select  DMSV.MaSV,DMSV.HoSV,DMSV.TenSV, DMKhoa.TenKhoa,DMSV.HocBong from DMSV
JOIN DMKhoa ON DMKhoa.MaKhoa=DMSV.MaKhoa 
WHERE TenKhoa<>'anh văn' and HocBong>(select sum(HocBong)from DMSV JOIN DMKhoa ON DMKhoa.MaKhoa=DMSV.MaKhoa WHERE TenKhoa='anh văn' GROUP BY DMSV.MaKhoa);

-- 48.Cho biết những sinh viên có học bổng lớn hơn bất kỳ học bổng của sinh viên học khóa anh văn
select  DMSV.MaSV,DMSV.HoSV,DMSV.TenSV, DMKhoa.TenKhoa,DMSV.HocBong from DMSV
JOIN DMKhoa ON DMKhoa.MaKhoa=DMSV.MaKhoa 
WHERE TenKhoa<>'anh văn' and HocBong>(select max(HocBong)from DMSV JOIN DMKhoa ON DMKhoa.MaKhoa=DMSV.MaKhoa WHERE TenKhoa='anh văn' GROUP BY DMSV.MaKhoa);

-- 49. ho biết sinh viên nào có điểm thi môn cơ sở dữ liệu lần 2 lớn hơn tất cả điểm thi lần 1 môn cơ sở dữ liệu của những sinh viên khác.
select  DMSV.MaSV,DMSV.HoSV,DMSV.TenSV,KetQua.Diem from DMSV JOIN KetQua ON KetQua.MaSV=DMSV.MaSV JOIN DMMH ON DMMH.MaMH=KetQua.MaMH
WHERE KetQua.LanThi=2 and DMMH.TenMH='cơ sở dữ liệu' and KetQua.diem>(select sum(KetQua.diem) from KetQua JOIN DMMH ON DMMH.MaMH=KetQua.MaMH WHERE DMMH.TenMH='cơ sở dữ liệu' and KetQua.LanThi=1 GROUP BY KetQua.MaMH );
 
 -- 52.Cho biết sinh viên chưa thi môn cơ sở dữ liệu.
 select * from DMSV where MaSV NOT IN(select MaSV from KetQua JOIN DMMH ON DMMH.MaMH=KetQua.MaMH WHERE DMMH.TenMH='cơ sở dữ liệu' GROUP BY MaSV);
